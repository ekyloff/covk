(function () {
  ...
  function renderTable(data) {
    ...
    if (!tbody || !data || !data.sections || !data.sections.length) {
      showError('Каталог пуст или повреждён. Проверьте содержимое publics.json.');
      return;
    }
  ...
  function loadJson(callback) {
    var onFail = function () {
      showError('Не удалось загрузить publics.json. Проверьте, что файл доступен.');
    };
